...
static QSharedPointer<AVDataManager> avDataManagerInstance;

QSharedPointer<AVDataManager> AVDataManager::instance(QObject *parent)
{
    if (avDataManagerInstance.isNull())
    {
        avDataManagerInstance = QSharedPointer<AVDataManager>(new AVDataManager(parent));

        // FIXME. test code.
//        avDataManagerInstance.data()->init();
    }
...
{
    setTitle("IU - 스물셋 Title Text");
    setFileName("AOA - 사뿐사뿐(Like a Cat) Music Video.mp4");
    setFrequency("99.9");
    setPsName("FM");
    setPresetIndex("23");
}
...
QString AVDataManager::currentTime() const
{
    return m_currentTime;
}

void AVDataManager::setCurrentTime(QString curTime)
{
    if (m_currentTime == curTime)
        return;
